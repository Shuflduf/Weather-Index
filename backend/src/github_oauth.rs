...
pub async fn handle_callback(
    ...
    let access_token = token_resp["access_token"]
        ...
        .ok_or_else(|| {
            make_error(
                StatusCode::BAD_GATEWAY,
                "No access_token in GitHub response".into(),
            )
        })?
        .to_string();
    ...
    let user_resp = client
        ...
        .map_err(|e| {
            make_error(
                StatusCode::BAD_GATEWAY,
                format!("GitHub user API failed: {e}"),
            )
        })?
        .json::<serde_json::Value>()
        .await
    ...
    let emails = client
        ...
        .map_err(|e| {
            make_error(
                StatusCode::BAD_GATEWAY,
                format!("GitHub emails API failed: {e}"),
            )
        })?
        .json::<Vec<serde_json::Value>>()
        .await
        .map_err(|e| make_error(StatusCode::BAD_GATEWAY, format!("Bad emails response: {e}")))?;
