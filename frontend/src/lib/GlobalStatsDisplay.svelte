<script lang="ts">
  import type { GlobalStatsCategory } from "$lib";
  import { defaultProperties } from "./properties";
  import { formatBig, formatSeconds, type RunReportWithUser } from "./RoR2";

  let { stats }: { stats: Record<GlobalStatsCategory, RunReportWithUser> } =
    $props();
</script>

{#if true}
  {@const statSheet = stats.AVG}
  <div class="flex flex-row flex-wrap">
    {#each Object.entries(statSheet) as [stat, value]}
